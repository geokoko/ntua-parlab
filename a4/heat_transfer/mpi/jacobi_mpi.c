...
int main(int argc, char ** argv) {
    ...
    // MPI_Status will be used later
    MPI_Status status;

    double omega; 			//relaxation factor - useless for Jacobi

    // t = time
    // t = total, c = computation
    // s = start, f = finish
    struct timeval tts,ttf,tcs,tcf;   //Timers: total-> tts,ttf, computation -> tcs,tcf
    // We need to add timers for converge
    ...
    // scattercounts[p] : number of blocks sent from the global matrix to process p
    // scatteroffset[p] : displacement (starting position) of process p's block inside the global padded matrix U,
    // expressed in units of the resized global_block datatype (i.e., in number of doubles)
    ...
    // We send to u_previous[1][1] and not [0][0] because those are ghost cells
    MPI_Scatterv(U0_ptr, scattercounts, scatteroffset, global_block, &(u_previous[1][1]), 1, local_block,
    0, MPI_COMM_WORLD);
    MPI_Scatterv(U0_ptr, scattercounts, scatteroffset, global_block, &(u_current[1][1]), 1, local_block,
    0, MPI_COMM_WORLD);
	...
	/*Fill your code here*/
    // It's easy to send a row to another process
    // But we need custom datatype to send a column, since arrays 
    // are row-major in C
    MPI_Datatype column;
    ...
    // Shift by 1 step in x direction (find north and south neighbor) and y direction (find west and east neighbor)
    // That's how we find the neighbor cells for the stensil
    // Since the grid is defined as non-periodic
	...
	*/
    ...
    for (t=0;t<T;t++) {
        ...
        // Send to your north neighbor your top row and receive its bottom row
        if (north != MPI_PROC_NULL) {
            MPI_Sendrecv(&u_previous[1][1], local[1], MPI_DOUBLE, north, 0, &u_previous[0][1], local[1],
                MPI_DOUBLE, north, 0, MPI_COMM_WORLD,&status);
        }

        // Send to your south neighbor your bottom row and receive its top row
        if (south != MPI_PROC_NULL) {
            MPI_Sendrecv(&u_previous[local[0]][1], local[1], MPI_DOUBLE, south, 0, &u_previous[local[0] + 1][1], local[1],
                MPI_DOUBLE, south, 0, MPI_COMM_WORLD, &status);
        }

        // Send to your east neighbor your right-most column and receive its left-most column
        if (east != MPI_PROC_NULL) {
            MPI_Sendrecv(&u_previous[1][local[1]], 1, column, east, 0, &u_previous[1][local[1] + 1], 1,
                column, east, 0, MPI_COMM_WORLD, &status);
        }

        // Send to your west neighbor your left-most column and receive its right-most column
        if (west != MPI_PROC_NULL) {
            MPI_Sendrecv(&u_previous[1][1], 1, column, west, 0, &u_previous[1][0], 1,
        ...
        if (t%C==0) {
            ...
            // Check whether every process converged (that's why we use MPI_MIN, so if even one hasn't converged we keep going)
    ...
    MPI_Gatherv(&u_current[1][1], 1, local_block, U0_ptr, scattercounts, scatteroffset, global_block, 0, MPI_COMM_WORLD);
