...
double euclid_dist_2_transpose(int numCoords, int numObjs, int numClusters,
...
void find_nearest_cluster(int numCoords,
	...
	/* Check if the thread ID is within bounds (if it is not, it should not do any work) */
	int tid = get_tid();
	...
	/* Find the nearest cluster for the object assigned to this thread */
	min_dist = euclid_dist_2_transpose(numCoords, numObjs, numClusters, objects, deviceClusters, tid, 0);
	index = 0;

	for (c = 1; c < numClusters; c++) {
		double dist = euclid_dist_2_transpose(numCoords, numObjs, numClusters, objects, deviceClusters, tid, c);
		if (dist < min_dist) {
			min_dist = dist;
			index = c;
		}
...
void kmeans_gpu(double *objects,      /* in: [numObjs][numCoords] */
	...
	double **newClusters = (double **) calloc_2d(numCoords, numClusters, sizeof(double));  // [numCoords][numClusters]
	...
	do {
		...
		/* Copy clusters to deviceClusters (host -> gpu) */
	...
	/* Update clusters using dimClusters. Be careful of layout! */
	for (i = 0; i < numClusters; i++) {
		for (j = 0; j < numCoords; j++) {
