...
void GaussSeidelSOR(double **u_previous, double **u_current, int X_min, int X_max, int Y_min, int Y_max,
                    double omega) {
    int i, j;
...
int main(int argc, char **argv) {
    ...
    struct timeval tcvs, tcvf;               // Timers for convergence checks
    double ttotal = 0, tcomp = 0, tconv = 0; // local timers
    ...
    //----Define datatypes for message passing----//
    MPI_Datatype column;
...
#define T 256
    for (t = 0; t < T; t++) {
        ...
        // Ensure ghost cells used by Gauss-Seidel come from the previous iteration
    ...
    if (rank == 0) {
        ...
        printf("GaussSeidelSOR X %d Y %d Px %d Py %d Iter %d ComputationTime %lf TotalTime %lf CommunicationTime %lf ConvergenceTime %lf midpoint %lf\n",
